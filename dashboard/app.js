...
const API = (() => {
  ...
  const ready = new Promise((res, rej) => {
    w.onmessage = e => {
      ...
      if (d.type === 'fatal') { rej(new Error(d.error)); const o = document.getElementById('boot'); if (o) o.innerHTML = `<div class="boot-box err">引擎載入失敗<br><small>${d.error}</small></div>`; return; }
...
const ROLE_ZH = { '보조': '輔助', '방해': '干擾', '치유': '治療', '수호': '守護', '전사': '戰士' };
const SPECIAL = { 10421: 4.5, 10401: 5.5 };
const EL_ORDER = ['fire', 'water', 'wood', 'light', 'dark'];
const EL_KR = { fire: '火', water: '水', wood: '木', light: '光', dark: '暗', none: '無' };
const ELEMENT_ZH = { '불': '火', '물': '水', '나무': '木', '빛': '光', '어둠': '暗', '무': '無' };
...
function histView() {               // 검색 필터 + 정렬 + 핀 상단고정
  ...
  const cmp = { date: (a, b) => b.id - a.id, 'date-asc': (a, b) => a.id - b.id,
    name: (a, b) => (a.name || a.label).localeCompare(b.name || b.label, 'zh-Hant'),
...
function makeLabel(team, turns, total) {   // 팀에서 라벨 재생성 (공유 코드에선 라벨을 빼고 이걸로 복원)
  ...
  return `${names} · ${turns} 回合 · ${fmtShort(total || 0)}`;
}
...
function renderHistory(selId) {
  ...
  if (!simHistory.length) { sel.innerHTML = '<option value="">— 無紀錄 —</option>'; return; }
  ...
  sel.innerHTML = (has ? '' : '<option value="" selected>— 選擇要載入的紀錄 —</option>') + opts;
}
...
function renderHistList() {
  ...
  list.innerHTML = view.length ? view.map(r => {
    const d = new Date(r.id).toLocaleString('zh-TW', { month: 'numeric', day: 'numeric', hour: '2-digit', minute: '2-digit' });
    ...
    return `<label class="hist-item${r.pinned ? ' pinned' : ''}"><input type="checkbox" data-id="${r.id}">
      ...
      <button class="hi-menu" data-menu="${r.id}" title="管理">⋮</button></label>`;
  }).join('') : `<div class="hist-empty">${histSearch ? '找不到符合的結果' : '尚無已儲存紀錄'}</div>`;
  updateHselCount();
}
...
function updateHselCount() {
  ...
  $('#hselCount').textContent = `已選 ${n} 筆 / 全部 ${simHistory.length} 筆`;
...
function bindHistory() {
  ...
  $('#hDelSel').onclick = () => {
    ...
    if (!del.length) return toast('沒有可刪除的紀錄（已鎖定的紀錄會排除）');
    if (!confirm(`要刪除所選 ${del.length} 筆嗎？（排除已鎖定紀錄）`)) return;
  ...
  $('#hDelOther').onclick = () => {
    ...
    const del = simHistory.filter(r => !ids.has(r.id) && !r.locked);
    if (!del.length) return toast('沒有可刪除的紀錄');
    if (!confirm(`保留所選 ${ids.size} 筆與已鎖定紀錄，刪除其餘 ${del.length} 筆嗎？`)) return;
    const dset = new Set(del.map(r => r.id));
    simHistory = simHistory.filter(r => !dset.has(r.id)); afterHistChange();
  };
}
function openHistMenu(btn, r) {
  ...
  m.innerHTML = `<button data-act="rename">✏️ 變更名稱</button>
    <button data-act="pin">${r.pinned ? '📌 取消釘選' : '📌 釘選到上方'}</button>
    <button data-act="lock">${r.locked ? '🔓 解除鎖定' : '🔒 鎖定'}</button>
    <button data-act="del" class="danger"${r.locked ? ' disabled' : ''}>🗑️ 刪除</button>`;
  ...
  m.onclick = e => {
    ...
    if (act === 'rename') { const nn = prompt('新名稱（留空則使用預設名稱）', r.name || r.label); if (nn !== null) { r.name = nn.trim() || undefined; afterHistChange(); } }
...
function importRecords(arr) {                 // 공통 머지 (성공 시 true)
  if (!Array.isArray(arr)) { toast('匯入失敗：格式不正確'); return false; }
...
async function openExportPop() {
  ...
  pop.innerHTML = `<div class="io-card"><button class="mc-close" data-ioclose>×</button>
    <div class="pp-head"><h3>匯出 <em>（已選 ${out.length} 筆）</em></h3></div>
    <button class="io-big" id="ioFile">📁 儲存為檔案</button>
    <div class="io-or">或以分享碼傳送</div>
    <textarea class="io-code" id="ioCode" readonly>${code}</textarea>
    <button class="io-big" id="ioCopy">📋 複製分享碼</button></div>`;
...
function openImportPop() {
  ...
  const pop = document.createElement('div'); pop.className = 'iopop';
  pop.innerHTML = `<div class="io-card"><button class="mc-close" data-ioclose>×</button>
    <div class="pp-head"><h3>匯入</h3></div>
    <button class="io-big" id="ioPickFile">📁 選擇檔案</button>
    <div class="io-or">或貼上分享碼</div>
    <textarea class="io-code" id="ioPaste" placeholder="將收到的分享碼貼在這裡"></textarea>
    <button class="io-big" id="ioPasteBtn">用分享碼匯入</button></div>`;
  document.body.appendChild(pop);
  $('#ioPickFile').onclick = () => $('#hImportFile').click();
  $('#ioPasteBtn').onclick = async () => {
    const v = $('#ioPaste').value.trim(); if (!v) return toast('請貼上分享碼');
    let arr;
    try { arr = JSON.parse(await decompressCode(v)); } catch { try { arr = JSON.parse(v); } catch { return toast('匯入失敗：分享碼不正確'); } }
...
function importHistory(e) {
  ...
  reader.onload = async () => {
    ...
    catch { try { arr = JSON.parse(await decompressCode(reader.result.trim())); } catch { toast('匯入失敗：不是有效的紀錄檔'); e.target.value = ''; return; } }
...
(function showLastUpdate() {
  ...
  fetch('version.json', { cache: 'no-store' }).then(r => r.ok ? r.json() : null).then(v => {
    if (!v || !v.updated) { el.textContent = '開發模式 · 本機'; return; }
...
function cmpCell(c, side) {
  if (!c) return `<div class="cmp-cell empty" data-side="${side}" data-add>＋ 新增</div>`;
  const d = c.damage == null ? `<span class="cc-d pend">?</span>` : `<span class="cc-d">${fmtShort(c.damage)}</span>`;
  return `<div class="cmp-cell el-${c.elementKey}" data-side="${side}" draggable="true" title="點擊設定印章、行動、替換">
...
function renderCmpLane() {
  ...
  const rows = cmpPairs.map((r, i) => {
    const mid = cmpPending ? `<div class="cmp-mid pend">↻</div>`
      : ((r.a && r.b) ? midHtml(r.a.damage, r.b.damage) : `<div class="cmp-mid solo">單獨</div>`);
  ...
  const addRow = (aAdd || bAdd)
    ...
    : '';
  let total;
  if (cmpPending) total = `<div class="cmp-pending">已變更，請按 <b>開始比較</b> 更新結果</div>`;
  else if (aN || bN) {
  ...
  const prioRow = (aN || bN) ? `<div class="cmp-priorow">
    <span>${aN ? `<button class="ct-prio" data-prio="a">⇅ 行動優先順序</button>` : ''}</span>
    <span>${bN ? `<button class="ct-prio" data-prio="b">⇅ 行動優先順序</button>` : ''}</span></div>` : '';
  $('#cmpBody').innerHTML = `<div class="cmp-colhead"><span class="ch-a">A</span><span>較高方 ◀▶ · ${cmpCommon.turns || ''} 回合</span><span class="ch-b">B</span></div>
  ...
  pop.innerHTML = `<div class="ci-card el-${c.elementKey}" style="--el:var(--${c.elementKey})">
    <div class="ci-head">
      <div class="ci-top"><img src="${icon(c.id)}" alt=""><div><h3>${esc(c.name)}</h3>
        <div class="ci-tags"><span class="tag el">${elementLabel(ch.element)}屬性</span><span class="tag">${roleLabel(ch.role)}</span><span class="tag">P${cf.position || c.position}</span></div></div></div>
      <div class="ci-hbtns"><button class="ci-swap" data-swap>⇄ 替換</button><button class="mc-close" data-ciclose>×</button></div>
    </div>
    <div class="ci-rows">
      <div class="ci-r tap" data-seal><span>印章強化</span><b class="ci-state ${cf.sealOn ? 'on' : ''}" id="ciSeal">${cf.sealOn ? 'ON' : 'OFF'} ▸</b></div>
      <div class="ci-r"><span>基礎攻擊力</span><b id="ciAtk">${fmt((ch.atk || 0) + ua)}${ua ? `<em>(+${fmt(ua)})</em>` : ''}</b></div>
      <div class="ci-r"><span>最大體力</span><b id="ciHp">${fmt((ch.hp || 0) + uh)}${uh ? `<em>(+${fmt(uh)})</em>` : ''}</b></div>
      <div class="ci-r tap" data-plan><span>行動</span><b class="ci-state" id="ciAct">${isManual ? '手動' : '自動'} ▸</b></div>
    </div></div>`;
  document.body.appendChild(pop);
  pop.onclick = e => {
...
function openSealPop(c) {
  ...
  pop.innerHTML = `<div class="sp-card"><button class="mc-close" data-spclose>×</button>
    <div class="pp-head"><img src="${icon(c.id)}" alt=""><h3>${esc(c.name)} · 印章強化</h3></div>
    <label class="toggle pp-toggle"><input type="checkbox" id="spOn"><span class="sw"></span>印章強化 <em>上限 ${fmt(limit)}（攻擊力+體力）</em></label>
    <div class="sp-body" id="spBody">
      <div class="seal-row"><span class="sl-lbl atk">攻擊力</span>
        <input type="range" id="spAtkR" min="0" max="${limit}" step="100"><input type="number" id="spAtkN" min="0" max="${limit}" step="100"></div>
      <div class="seal-row"><span class="sl-lbl hp">體力</span>
  ...
  const sync = atk => {
    ...
    $('#spRatio').innerHTML = `攻擊力 <b class="atk">${ap}%</b> : 體力 <b class="hp">${100 - ap}%</b>`;
...
function openAddPop(side) {
  ...
  const list = Object.values(CHARS).sort((x, y) => (x.name || '').localeCompare(y.name || '', 'zh-Hant'));
  ...
  pop.innerHTML = `<div class="sw-card"><button class="mc-close" data-swclose>×</button>
    <div class="pp-head"><h3>新增角色 <em>（比較隊伍 ${side === 'a' ? 'A' : 'B'}）</em></h3></div>
  ...
  pop.onclick = e => {
    ...
    if (b && !b.disabled) {
      ...
      if (idx < 0) { if (cmpTeam[side].filter(Boolean).length >= 5) return toast('比較隊伍已滿（最多 5 名）'); idx = cmpTeam[side].length; }
...
function openSwapPop(c) {
  ...
  const pop = document.createElement('div'); pop.className = 'swappop';
  pop.innerHTML = `<div class="sw-card"><button class="mc-close" data-swclose>×</button>
    <div class="pp-head"><h3>替換角色 <em>（比較隊伍 ${c.side === 'a' ? 'A' : 'B'}）</em></h3></div>
    <div class="sw-grid"><button class="sw-ic sw-none" data-id="none"><span class="sw-x">✕</span><span>移除</span></button>${grid}</div></div>`;
  document.body.appendChild(pop);
  pop.onclick = e => {
    const b = e.target.closest('.sw-ic');
    if (b && !b.disabled) {
...
function openPlanPopup(c) {
  ...
  pop.innerHTML = `<div class="pp-card"><button class="mc-close" data-ppclose>×</button>
    <div class="pp-head"><img src="${icon(c.id)}" alt=""><h3>${esc(c.name)} · 行動</h3></div>
    <label class="toggle pp-toggle"><input type="checkbox" id="ppOn"><span class="sw"></span>手動指定行動 <em>（關閉則自動）</em></label>
    <div class="pp-legend"><span class="ro-a a평">普</span>普攻<span class="ro-a a궁">必</span>必殺<span class="ro-a a방">防</span>防禦</div>
  ...
  $('#ppOn').onchange = () => {
    ...
    const a = $('#ciAct'); if (a) a.textContent = (c.cfg.usePlan ? '手動' : '自動') + ' ▸';   // 상세 팝업 라벨 즉시 갱신
...
function renderPlanPop(c) {                 // 본 플래너와 동일: CD 게이팅·방어 CD감소·궁 재배치 적용
  ...
  for (let ti = 0; ti < turns; ti++) {
    ...
    for (let a = 0; a < apt; a++) {
      ...
      const btn = (k, l) => {
        ...
        return `<button class="a${k}${act === k ? ' on' : ''}" data-idx="${idx}" data-a="${k}"${(!on || lockUlt) ? ' disabled' : ''}>${actionLabel(l)}</button>`;
      };
  ...
  if (rules) {
    const ruleTxt = apt > 1
      ? `每回合 <b style="color:var(--gold)">${apt} 次行動</b> · 必殺每回合限 1 次（不可連續必殺）· 額外行動固定為普攻`
      : `必殺 CD <b style="color:var(--gold)">${meta.fatalCd} 回合</b> · 首次使用 <b style="color:var(--gold)">${meta.firstFatal} 回合</b> · CD 未好時必殺不可使用`;
    rules.innerHTML = `<span>${ruleTxt}</span><span class="plan-fill">
      <button data-fill="평"${on ? '' : ' disabled'}>全部普攻</button><button data-fill="방"${on ? '' : ' disabled'}>全部防禦</button></span>`;
  ...
  grid.onclick = on ? (e => {
    ...
    if (a === '궁' && meta.cdDefendReduce > 0) {      // 모이루·히토하: 앞턴 방어+입질 평타 자동 배치
      ...
      toast(`${meta.name}: 為了縮短必殺 CD，已自動將前一回合設為 <b>防禦</b>，並在更前面配置觸發用 <b>普攻</b>`);
...
function openPrioPop(side) {                // 비교군별 행동 우선순위 + 특정 턴 오버라이드
  ...
  pop.innerHTML = `<div class="pr-card"><button class="mc-close" data-prclose>×</button>
    <div class="pp-head"><h3>行動優先順序 <em>（比較隊伍 ${side === 'a' ? 'A' : 'B'}）</em></h3></div>
    <div class="pr-sub">行動順序 <em>（拖曳或 ▲▼）</em></div>
    <ol class="prio" id="prPrio"></ol>
    <div class="pr-sub">只調整特定回合 <em>（選擇回合後變更順序）</em></div>
    <div class="turn-chips" id="prChips"></div>
    <div id="prEditor"></div>
    <button class="btn-ghost sm" id="prReset" style="margin-top:10px">全部恢復預設</button></div>`;
  document.body.appendChild(pop);
  ...
  function renderEditor() {
    ...
    const label = sel.length === 1 ? `${first} 回合` : `${sel.length} 個回合（${sel.join('·')}）`;
    ed.innerHTML = `<div class="te-head"><b>${label}</b> 行動順序 — ${anyHas ? '已變更' : '使用預設'}${sel.length > 1 ? ' <em>以相同順序批次套用</em>' : ''}</div>
      ...
      ${anyHas ? '<button class="btn-ghost sm" id="prClearTurn">所選回合恢復預設</button>' : ''}`;
...
function renderCmpChart() {
  ...
  $('#cmpChart').innerHTML = `<div class="cc-title">各回合累積傷害 <em>（以 ${n} 回合為基準）</em></div>
    ...
    <div class="cc-leg"><span class="lg a">A 總計 ${fmtShort(ca[n-1])}</span><span class="lg b">B 總計 ${fmtShort(cb[n-1])}</span><span class="cc-hh">將滑鼠移到長條上查看各回合差異</span></div>`;
}
function cmpCursor(plot, i) {
  ...
  tip.innerHTML = `<div class="ct-t">${i + 1} 回合</div>
    <div class="ct-l"><span class="d-a">A</span>${fmt(a)}</div><div class="ct-l"><span class="d-b">B</span>${fmt(b)}</div>
    <div class="ct-d ${a === b ? 'eq' : aWin ? 'win-a' : 'win-b'}">${a === b ? '相同' : `${aWin ? 'A' : 'B'} +${fmtShort(diff)} · +${pct}%`}</div>`;
  tip.hidden = false;
...
async function runCompare() {
  ...
  if (!ra || !rb) { $('#cmpBody').innerHTML = `<div class="cmp-hint">請選擇要比較的對象</div>`; return; }
  if (ra.id === rb.id) { $('#cmpBody').innerHTML = `<div class="cmp-hint">請選擇兩筆不同紀錄</div>`; return; }
  loadCmpTeam('a', ra); loadCmpTeam('b', rb);
  $('#cmpRun')?.classList.remove('dirty');
  $('#cmpBody').innerHTML = `<div class="cmp-hint"><span class="spin"></span> 正在重新執行兩組隊伍…</div>`;
  try {
    ...
    renderCmpLane();
  } catch (e) { $('#cmpBody').innerHTML = `<div class="cmp-hint">比較失敗：${esc(e.message || '錯誤')}</div>`; }
}
function bindCompare() {
  ...
  $('#cmpBtn').onclick = () => {
    ...
    $('#cmpA').innerHTML = `<option value="">＋ 比較隊伍 A（空編成）</option>` + opts();
    $('#cmpB').innerHTML = `<option value="">＋ 比較隊伍 B（空編成）</option>` + opts();
...
function buildFilters() {
  ...
  f.innerHTML = mk('all', '全部') + EL_ORDER.map(e => mk(e, EL_KR[e])).join('');
...
function mvArrows(k, len) {   // 터치용 위/아래 버튼 (드래그가 안 되는 모바일 대비)
  return `<span class="mv-col">
    <button class="mv" data-mv="up" data-k="${k}"${k === 0 ? ' disabled' : ''} aria-label="向上">▲</button>
    <button class="mv" data-mv="dn" data-k="${k}"${k === len - 1 ? ' disabled' : ''} aria-label="向下">▼</button></span>`;
}
function renderPrio() {
  const ord = teamOrder();
  ...
  ed.innerHTML = `<div class="te-head"><b>${label}</b> 行動順序 — ${anyHas ? '已變更' : '使用預設'}${sel.length > 1 ? ' <em>以相同順序批次套用</em>' : ''}</div>
    <ol class="prio">${ord.map((p, k) => { const c = CHARS[team[p - 1].id];
      return `<li draggable="true"><span class="ord">${k + 1}</span><img class="pic el-${c.elementKey}" src="${icon(team[p - 1].id)}" alt="" draggable="false">
        <span class="nm">${c.name}</span>${mvArrows(k, ord.length)}</li>`; }).join('')}</ol>
    ${anyHas ? '<button class="btn-ghost sm" id="clearTurn">所選回合恢復預設</button>' : ''}`;
...
function bindSettings() {
  ...
  $('#forceProc').onclick = () => {
    ...
    toast(forceProc ? '機率 100% 模式 ON<br>· 所有機率型技能強制 100%' : '機率 100% 模式 OFF');
  };
  $('#hp10Btn').onclick = () => {
    hp10 = !hp10; $('#hp10Btn').classList.toggle('on', hp10);
    toast(hp10 ? '體力 10% 模式 ON<br>· 假人體力固定 10%（觸發低 HP 條件）' : '體力 10% 模式 OFF');
  };
...
async function openModal(i) {
  ...
  card.innerHTML = `<button class="mc-close" data-close>×</button>
    <div class="mc-top"><img src="${icon(s.id)}" alt="">
      <div class="info"><h2>${c.name}</h2><div class="tags">
        <span class="tag el">${elementLabel(c.element)}屬性</span><span class="tag">${roleLabel(c.role)}</span><span class="tag">P${i + 1}</span></div></div></div>
    <div class="mc-seal${s.sealOn ? ' on' : ''}" id="mcSeal">
      <label class="toggle seal-head"><input type="checkbox" id="sealOn" ${s.sealOn ? 'checked' : ''}><span class="sw"></span>印章強化 <em>上限 ${fmt(limit)}（攻擊力+體力）</em></label>
      <div class="seal-body">
        <div class="seal-row"><span class="sl-lbl atk">攻擊力</span>
    ...
    <div class="mc-stats">
      <div class="s"><label>基礎攻擊力 <em id="stAtkAdd"></em></label><b class="num" id="stAtk">${fmt(c.atk + s.sealAtk)}</b></div>
      <div class="s"><label>最大體力 <em id="stHpAdd"></em></label><b class="num" id="stHp">${fmt(c.hp + s.sealHp)}</b></div></div>
    <div class="mc-ctrl">
      <label class="field"><span>技能等級 <b id="sklVal" style="color:var(--gold)">${s.skill}</b></span>
        <div class="row"><input type="range" id="skl" min="1" max="10" value="${s.skill}"></div></label>
      <label class="toggle"><input type="checkbox" id="rune" ${s.rune ? 'checked' : ''}><span class="sw"></span>解除印章（符文）</label>
    </div>
    <div class="field">
      <label class="toggle" style="margin-bottom:10px"><input type="checkbox" id="usePlan" ${s.usePlan ? 'checked' : ''}><span class="sw"></span>手動規劃各回合行動 <em style="margin-left:4px">（關閉則自動）</em></label>
      <div id="plannerWrap" ${s.usePlan ? '' : 'hidden'}>
        <div class="plan-legend">
          <span>${(c.actionsPerTurn || 1) > 1
            ? `每回合 <b style="color:var(--gold)">${c.actionsPerTurn} 次行動</b> · <b style="color:var(--gold)">必殺每回合限 1 次</b>（不可連續必殺）· 額外行動固定為普攻`
            : `必殺 CD <b style="color:var(--gold)">${c.fatalCd} 回合</b> · 首次使用 <b style="color:var(--gold)">${c.firstFatal} 回合</b> — <b style="color:var(--gold)">必殺</b> 在 CD 未好時不可使用`}</span>
          <span class="plan-fill"><button data-fill="평">全部普攻</button><button data-fill="방">全部防禦</button></span>
        </div>
        <div class="planner" id="planner"></div>
      </div>
    </div>
    <div class="skills" id="skills"><div class="empty-state"><span class="spin"></span>正在載入技能…</div></div>`;
  m.hidden = false;
  ...
  const syncSeal = atk => {
    ...
    const ap = Math.round(atk / limit * 100);
    $('#sealRatio', card).innerHTML = `攻擊力 <b class="atk">${ap}%</b> : 體力 <b class="hp">${100 - ap}%</b>`;
  ...
  wrap.innerHTML = Array.from({ length: n }, (_, ti) => {
    ...
    for (let a = 0; a < apt; a++) {
      ...
      const b = (k, l) => {
        ...
        return `<button class="a${k} ${act === k ? 'on' : ''}" data-idx="${idx}" data-a="${k}"${lockUlt ? ' disabled' : ''}>${actionLabel(l)}</button>`;
      };
...
function renderSkills(detail, lvl) {
  ...
  wrap.innerHTML = detail.skills.filter(sk => {
    ...
    const slotName = skillSlotLabel(sk.slotKr);
    const slotEl = ic ? `<img class="slot-ic" src="${ic}" alt="${slotName}" title="${slotName}">` : `<span class="slot">${slotName}</span>`;
...
function notifyUpdate() {        // 새 배포 감지 시 기존 접속자에게 띄우는 고정 알림(눌러서 새로고침)
  ...
  t.innerHTML = `🔄 已有新版本發布，<b>點擊重新整理</b>`;
...
async function run(save = true) {
  ...
  if (unplanned) toast(`注意：建議設定 ${CHARS[unplanned.id].name} 的各回合行動`);
  const hpSchedChar = picked.find(s => CHARS[s.id].hpSchedule);   // 카라트: 적 HP 의존
    if (hpSchedChar && !hp10) toast(`${CHARS[hpSchedChar.id].name} 在隊時，敵方 HP% 會依進行回合四等分逐步下降（前 1/4 ≥75% → 最後 1/4 &lt;25%）`);
  const btn = $('#runBtn'); btn.classList.add('busy'); btn.querySelector('span').innerHTML = '<span class="spin"></span>計算中…';
  const cfg = {
  ...
  } catch (err) { alert('模擬錯誤：' + err.message); }
  finally { btn.classList.remove('busy'); btn.querySelector('span').textContent = '執行模擬'; }
}
...
function renderResults(d) {
  ...
  $('#hTotalRange').textContent = multi ? `最小 ${fmtShort(m.totalMin)} ~ 最大 ${fmtShort(m.totalMax)}` : '';
  $('#hDpsRange').textContent = multi ? `最小 ${fmtShort(m.dpsMin)} ~ 最大 ${fmtShort(m.dpsMax)}` : '';
  $$('.hero-num label em').forEach(e => e.style.display = multi ? '' : 'none');
  $('#topMeta').textContent = multi
    ? `${m.runs} 次 · 平均 ${fmtShort(m.total)} · ±${fmtShort(m.totalStd)}`
    : '機率 100% · 確定性';
  $('#logOrder').textContent = '行動順序：' + m.order.join(' → ') + (multi ? `  ·  紀錄為接近平均的一次樣本` : '');
  ...
  $('#rank').innerHTML = d.perChar.map((c, i) => `
    <div class="rbar el-${c.elementKey}" style="--el:var(--${c.elementKey})">
      ...
      <div class="track"><div class="fill" style="width:0"></div>
        <div class="lab">${c.name}<span class="role">${roleLabel(c.role)}</span></div></div>
      <div class="val">${fmt(c.damage)}<small>${c.share}%${c.healing ? ' · 治療 ' + fmt(c.healing) : ''}</small></div>
    </div>`).join('');
  ...
  $('#chart').innerHTML = d.chart.map(t => {
    ...
    const tip = `<div class="tip"><div class="tip-h">${t.turn} 回合 <b>${fmt(t.total)}</b></div>${rows || '<div class="tip-row empty">無傷害</div>'}</div>`;
...
function renderLog(d) {
  ...
  $('#log').innerHTML = Object.keys(turns).map(Number).sort((a, b) => a - b).map(tn => {
    ...
    return `<div class="turn"><div class="turn-h"><b class="tn-num">${tn} 回合</b>
      <span class="sum"><span class="s-val dmg">${fmt(dmg)}<em>傷害</em></span>${heal ? `<span class="s-val heal">${fmt(heal)}<em>治療</em></span>` : ''}${bar ? `<span class="s-val bar">${fmt(bar)}<em>護盾</em></span>` : ''}</span>
...
function renderActions(evs) {
    ...
    if (kind === '受擊') {
      ...
      hitg.push(`<div class="act hitgrp">
        <div class="act-h"><span class="atkr">${atkBy}</span><span class="hit-arrow">→</span>
          ${pic}<span class="an">${lines[0].actor}</span><span class="ak ak-hit">受擊</span>
          <span class="at">${total ? fmt(total) : '<i>無反應</i>'}</span></div>
        <div class="act-b">${hits.map(renderHit).join('')}${body.map(renderND).join('')}</div></div>`);
      return;
    }
    if (!hits.length && !nd.length) return;
    const pic = id ? `<img class="ai el-${c ? c.elementKey : 'none'}" src="${icon(id)}" alt="">` : '<span class="ai sys">敵</span>';
    const kindEl = kind ? `<span class="ak ak-${KCLASS[kind] || ''}">${kindText}</span>` : '';
    ally.push(`<div class="act">
      <div class="act-h">${pic}<span class="an">${id ? lines[0].actor : '敵方行動'}</span>${kindEl}
        <span class="at">${total ? fmt(total) : '<i>增益/層數</i>'}</span></div>
  ...
  if (hitg.length) {              // 적 공격은 한 줄로 접고, 누르면 피격 그룹들이 펼쳐짐
    html += `<div class="act enemy-grp">
      <div class="act-h"><span class="ai sys">敵</span><span class="an">敵方攻擊</span>
        <span class="ak ak-hit">${hitg.length} 次受擊</span>
        <span class="at">${hitTotal ? fmt(hitTotal) : '<i>無反應</i>'}</span></div>
...
function renderND(l) {
  ...
  if (l.detail && l.detail.calc === 'flatAtk') {
    const d = l.detail;
    const baseAtk = chan('基礎ATK', d.baseAtk) || '<span class="chan-static">基礎ATK +0%</span>';
    const pctEl = l.srcId
      ? `<span class="skchip" data-sid="${l.srcId}" data-sn="${(l.srcSkill || '').replace(/"/g, '&quot;')}">${d.pct}% 倍率</span>`
      : `<span class="sk">${d.pct}%</span>`;
    return `<div class="ndl ${cls} fatk">
      <div class="ndl-h">${text}<span class="ndl-caret">▾</span></div>
      <div class="ndl-calc"><b>${fmt(d.val)}</b> = (${fmt(d.base)} × (1 + ${baseAtk})) × ${pctEl} <em>自身基礎ATK</em></div></div>`;
  }
  ...
  if (l.detail && (l.detail.kind === 'heal' || l.detail.kind === 'barrier')) {
    ...
    if (d.baseLabel === 'ATK') {
      inner = fmt(d.base);
      const bc = chan('基礎ATK', d.baseAtk), ac = chan('ATK', d.atk);
      if (bc) inner += ' × ' + bc;
      if (ac) inner += ' × ' + ac;
      if (sumv(d.flat || [])) inner += ' + ' + chan('固定', d.flat, '');
      inner = '(' + inner + ')';
    } else { inner = `${fmt(d.baseTotal)} ${d.baseLabel}`; }   // 최대HP 기반
    const pctEl = l.srcId
      ? `<span class="skchip" data-sid="${l.srcId}" data-sn="${(l.srcSkill || '').replace(/"/g, '&quot;')}">${d.skillPct}% 倍率</span>`
      : `<span class="sk">${d.skillPct}% 倍率</span>`;
    const eff = (d.eff || []).length ? ' × ' + chan('效果', d.eff) : '';
    const recv = d.healRecv ? ` × <span class="chan-static">受到治療 +${d.healRecv}%</span>` : '';
    return `<div class="ndl ${cls} fatk">
      <div class="ndl-h">${text}<span class="ndl-caret">▾</span></div>
...
function renderHit(l) {
  ...
  if (sumv(d.flat)) inner += ' + ' + chan('固定', d.flat, '');
  const sk = d.skillId
    ? `<span class="skchip" data-sid="${d.skillId}" data-sn="${(d.skillName || '').replace(/"/g, '&quot;')}">技能 ${d.skillPct}% 倍率</span>`
    : `<span class="sk">技能 ${d.skillPct}% 倍率</span>`;
  const dealt = d.dealt.length ? ' × ' + chan('造成傷害', d.dealt) : '';
  const eff = d.eff.length ? ' × ' + chan(d.effLabel, d.eff) : '';
  // 받뎀증 = 일반 × 속성 (별개 곱연산 채널이라 따로 표시). 구버전(d.taken)은 합쳐서 표시.
  const tg = (d.takenG || []).length ? ' × ' + chan('受到傷害', d.takenG) : '';
  const tp = (d.takenP || []).length ? ' × ' + chan('屬性受到傷害', d.takenP) : '';
  const takenOld = (d.taken || []).length ? ' × ' + chan('受到傷害', d.taken) : '';
  // 지속(도트) 전용 채널 — DoT 틱에만 표시
  const dd = (d.dotDealt || []).length ? ' × ' + chan('持續傷害增加', d.dotDealt) : '';
  const dt = (d.dotTaken || []).length ? ' × ' + chan('受到持續傷害', d.dotTaken) : '';
  // 속성 상성 배율 (1.0 아닐 때만): 상성 ×1.5(초록) / 역상성 ×0.75(빨강)
  const em = d.elemMult;
  const elem = (em && em !== 1)
    ? ` × <span class="elemx ${em > 1 ? 'adv' : 'dis'}">${em > 1 ? '剋制' : '被剋'} ×${em}</span>` : '';
...
function showSource(chip) {
  ...
  pop.innerHTML = `<div class="sp-h">${label} 來源</div>` + comps.map(c => {
    const ch = CHARMAP[c.by];
    const pic = c.by ? `<img src="${icon(c.by)}" class="el-${ch ? ch.elementKey : 'none'}">` : '<span class="np"></span>';
    const tag = c.cond ? ` <em>（${c.cond} 時）</em>` : c.el ? ` <em>（${elementLabel(c.el)}）</em>` : '';
    return `<div class="sp-row" data-id="${c.by}" data-skill="${(c.skill || '').replace(/"/g, '&quot;')}">
      ${pic}<span class="sp-v">${c.v > 0 ? '+' : ''}${c.v}${label === '固定' ? '' : '%'}</span>
      <span class="sp-s">${c.skill || '基本'}${tag}</span></div>`;
  }).join('') + '<div class="sp-tip">點擊技能可展開說明</div>';
  document.body.appendChild(pop);
...
async function openSkillFromSource(id, skillName) {
  ...
  const slotName = skillSlotLabel(sk.slotKr);
  const slotTag = ic ? `<img class="slot-ic" src="${ic}" alt="${slotName}" title="${slotName}">` : `<span class="tag el">${slotName}</span>`;
  card.innerHTML = `<button class="mc-close" data-close>×</button>
