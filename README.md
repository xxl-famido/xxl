...
전투 설정 머리말의 버튼으로 길드전 방탈출의 **제단 효과**와 **매 턴 피해**를 전투에 얹어 검증할 수 있습니다.
두 설정은 같은 자리(오른쪽 패널)에 열리는 탭이라 한 번에 하나만 보이며 서로 전환됩니다.

<div align="center">
<img src="docs/images/altar-btn.png" width="300" alt="제단 · 턴 피해 버튼">
</div>

#### 길드 제단 설정

- **별 제단**은 켤수록 불리해지는 페널티(받는 치료 감소·필살기 최대 CD 증가 등), **달 제단**은 켤수록 유리해지는 축복입니다. 같은 ‘활성화’라도 의미가 반대입니다.
- 층은 **1층부터 순서대로만** 켤 수 있고, 위층을 켜면 아래층도 함께 켜집니다. 각 효과는 개별로 껐다 켤 수 있습니다.
- **궁극기 사용 방식**(정해진 턴 / 정해진 턴만 / 준비되면 바로)과 **연동(궁 맞추기)** 은 행동 고급 설정의 탭에 있고 항상 적용됩니다. 연동 멤버는 ‘같이 궁’ 외에 **‘방어/평타 → 앵커가 준 추가 행동에서 궁’**(예: 마타야 방어 → 욱영 궁 → 마타야 궁)을 고를 수 있습니다. 우선순위는 **턴별 타임라인 → 연동 → 궁극기 사용 방식 → 추가 행동** 순입니다.
- **확률 쿨 감소 성공 가정**(궁극기 사용 방식 옵션): 확률로 쿨이 줄어드는 효과(길드 제단 등)를 굴리지 않고, 계획·연동으로 정한 궁 턴에 필요한 만큼만 터진 것으로 계산합니다. 예: 3쿨 캐릭터 3·6·9·12턴. 결과 머리말에 그 흐름이 그대로 나올 확률이 표시됩니다. 턴별 계획의 **첫 궁 당기기**가 이 사이클을 채워 줍니다.

<div align="center">
<img src="docs/images/altar-panel.png" width="360" alt="길드 제단 설정 패널">
&nbsp;&nbsp;
<img src="docs/images/altar-sync.png" width="360" alt="궁극기 맞추기 그룹">
</div>

#### 턴 피해 설정

매 턴 아군 전체가 **최대 HP의 일정 %**만큼 피해를 받게 합니다. 자기 HP에 반응하는 캐릭터(무명 등)와 **힐러·부활**의 가치를 확인할 때 씁니다.

- 적 페이즈가 끝날 때 적용되며, **배리어가 먼저 흡수**하고 방어한 턴은 절반만 받습니다. 받는 데미지 증감도 반영됩니다.
- **HP가 0이 되면 전투불능이 되어 이탈**합니다(이후 행동·타격·버프에서 빠져 딜이 끊깁니다). 피격 데미지 설정도 동일하며, 힐러가 HP를 유지해 주는지가 결과에 그대로 드러납니다.
- **부활** — 기리안의 도장(룬)은 필살기 발동 시 일정 확률로 사망한 아군 한 명을 최대 HP 25%로 되살립니다(확률 밴드에 반영).
- **피해 대상 (아군 수)** — 매 턴 피해를 받는 아군 수를 1~5인 중에 고릅니다(기본 전체). 전체보다 적게 두면 **현재 HP%가 높은 아군부터** 맞습니다(같으면 랜덤).
- **고급 · 턴별로 다르게**를 켜면 턴마다 피해 %를 따로 지정할 수 있습니다(비운 칸은 기본값).

<div align="center">
<img src="docs/images/tdmg-panel.png" width="360" alt="턴 피해 설정">
&nbsp;&nbsp;
<img src="docs/images/tdmg-adv.png" width="330" alt="턴별 피해 %">
</div>
