...
<img src="docs/images/battle-settings.png" width="320" align="right" alt="전투 설정">

- **확률 100% 모드** — 모든 확률형 버프·공격을 무조건 발동시켜 계산
- **반복 횟수** — 평균·최소·최대값 산출 (값이 클수록 정확하지만 느림)
- **진행 턴 수** 조절
- **더미 체력 10% 모드** — 딸피 구간 데미지 검증용 고정 체력 옵션
- **더미 속성** — 무·불·물·풀·빛·어둠 선택으로 상성(×1.5)·역상성(×0.75) 상황 확인 (무속성은 단순 데미지 계산용)
...
- **적 더미 수** 지정
- **아군 피격 횟수** — `0`(아군을 타격하지 않음), `1~5`(적 N명이 개별 공격), `전체`(아군 전체 1회 동시 피격). 반격 횟수 등 결과가 달라집니다.
...
턴별 행동 순서를 드래그로 지정하며, 특정 턴만 다른 순서를 적용할 수도 있습니다.
...
<div align="center">
<img src="docs/images/damage-trace.png" width="420" alt="손상 추적">
&nbsp;&nbsp;
